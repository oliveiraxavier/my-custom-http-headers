...
-   **Privacidade em Primeiro Lugar**: Nenhuma informação é enviada para servidores externos. Todos os dados, incluindo projetos e headers, são armazenados localmente no seu navegador.
...
-   `<all_urls>`: Permite que a extensão modifique os headers para qualquer site que você visitar. Esta permissão é necessária para que a funcionalidade principal seja aplicada globalmente.
...
2.  Abra seu navegador e navegue até a página de extensões:
    ...
    -   **Chrome/Edge**: WIP
3.  Ative o "Modo de Desenvolvedor" (geralmente um botão de alternância no canto superior direito).
4.  Clique em "Carregar sem compactação" (ou "Load Temporary Add-on" no Firefox) e selecione a pasta `chrome` (ou a raiz do projeto onde o `manifest.json` está localizado).
